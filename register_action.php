 ...
 $user_check_query = "SELECT * FROM registration WHERE phone='$phn' OR email='$email' LIMIT 1";
  ...
  if ($user) { // if user exists
    if ($user['phone'] === $phn) {
      ...
      header("Location: register.php?alert=Phone number already exists..");
    }

    if ($user['email'] === $email) {
    
      header("Location: register.php?alert=Email already exists..");
    }
  }
    ...
    else
    {
      ...
      $date=date("d-m-y");
      $oneYearOn = date('Y-m-d', strtotime($date. ' + 365 days'));
        ...
        $varsql="INSERT INTO `registration`(`app_type`, `name`, `address`, `pan`, `email`, `phone`, `gender`, `dob`, `district`, `image`, `proof`, `education_proof`, `education_proof1`, `education_proof2`, `sign`, `experience`, `issue_date`, `exp_date`, `pass`, `r_status`, `status`) VALUES ('$app','$name','$address','$pan','$email','$phn','$gender','null','$district','$img','$proof','null','null','$edu3','$sign','$exp','$date','null','$pwd','pending','contractor')";
...
$varresult=mysqli_query($con,$varsql);
  ...
  VALUES ('$email','$pwd','contractor','$z','1')");

   header("Location: register.php?alert=Successfully Register You Can Login Now..");
